...
class MyCmdInterpreter(cmd.Cmd):
    ...
    def do_create(self, arg):
        ...
        class_name = args[0]
        attributes = args[1:]
        timeOfChange = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        if class_name == 'User':
            user_id = str(User.latest)
            user_name, email = attributes
            new_user = User(user_id=user_id, username=user_name, email=email, timeOfChange=str(timeOfChange))
            if new_user:
            ...
            new_state = State(state_id=state_id, state_name=state_name, timeOfChange=str(timeOfChange))
            if new_state:
            ...
            new_city = City(city_id=city_id, city_name=city_name, state_name=state_name, timeOfChange=str(timeOfChange))
            if new_city:
            ...
            new_place = Place(place_id=place_id, place_name=place_name, city_name=city_name, timeOfChange=str(timeOfChange))
            if new_place:
                file_storage.add_place(new_place)
                file_storage.save()
                print("New place Created!!")
        else:
            print("Class specified is not present. Please try again.")
